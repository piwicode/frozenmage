...
// All collision operations are performed on rounded coordinate to avoid entering the walls.
// TODO: review the code to align xx and yy on integers when colliding.
if place_meeting(floor(xx + xs), floor(yy), obj_solid) {
  while(!place_meeting(floor(xx + sign(xs)), floor(yy), obj_solid)) xx += sign(xs);
  xs = 0;
}
